...
```{r}
# construction
zarr <- file.path(path, "images", "raccoon")
(ia <- readArray(zarr))
```
...
ps <- lapply(c(-300, 0, 300), \(t) 
    translateElement(image(spd), c(t, 0)))
...
```{r}
# spin the raccoon around
ps <- lapply(c(0, 90, 180), \(t) 
    rotateElement(image(spd), t))
wrap_plots(lapply(ps, plotElement))
```
...
ps <- lapply(c(1, 2/3, 1/2), \(t) 
    scaleElement(image(spd), c(1, 1, t)))
wrap_plots(lapply(ps, plotElement))
```
