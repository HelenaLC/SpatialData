...
.validateImageArray <- function(obj) {
  msg <- NULL
...
.validateSpatialData <- function(obj) {
  ...
  if (length(obj$labels)) {
    if (!all(vapply(obj$labels, is_ia, logical(1))))
      msg <- c(msg, "'labels' should be a list of 'ImageArray's")
  }
  if (length(obj$shapes)) {
    if (!all(vapply(obj$shapes, is_df, logical(1))))
      msg <- c(msg, "'shapes' should be a list of 'DFrame's")
  }
